...
tip is the main chain tip when the page was generated. A block's info card
offers stale blocks for download from the instance's API; `tree.html` points
that link at the full block in this repository instead, or drops it where the
dataset has only the header.

If a fork-observer change makes `blocktree.js` expect something new from
`main.js`, or renames what `tree.html` hooks into (`draw`, `onBlockClick`,
`recalc_tip_boxes`, `descLayer`, the `.tip-info-row` labels and the info
card's download links), `tree.html` has to follow. The browser console shows
the resulting error.
